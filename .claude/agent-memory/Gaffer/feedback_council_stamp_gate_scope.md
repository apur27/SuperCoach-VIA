...
**Stamp-format gotcha (2026-06-19):** the gate greps for the literal `<!-- council-pipeline:` ON ONE LINE. If you open the comment with `<!--` on its own line and `council-pipeline:` on the next, the grep fails and the commit is blocked as "missing stamp" even though the stamp is present. Keep `<!-- council-pipeline:` together on a single line. The verdict labels MUST carry a colon: the gate greps for `DataSentinel:` and `Skeptic:` literally — a stamp written `DataSentinel PASS` (space, no colon) fails as "missing DataSentinel: line" even though PASS is present. Write `DataSentinel: PASS ... | Skeptic: PASS | Gaffer: SHIP`. The gate also only checks that the FIRST `DataSentinel:` line contains "PASS" and the FIRST `Skeptic:` line contains "PASS" (case-insensitive) — so "Skeptic:PASS_WITH_CONCERNS" satisfies it, and a "DataSentinel: FAIL ... -> re-gate DataSentinel:PASS" single line passes because "PASS" is present on that line. Also note `core.hooksPath` may be unset (defaults to `.git/hooks`); the active hook there IS the council gate, a copy of the committed `.githooks/pre-commit`.
