...
4. **"Already fixed" is a claim to verify, not accept.** On the R21 third pass the operator stated the line-28 note had been "rewritten and cleared" — the text on disk was byte-identical to the version I had BLOCKed, still carrying "translating directly into" and "clearest example this round". The Ladder fix (line 24) *had* landed, so the report was half-true, which is what made it credible. **How to apply:** on any re-review, diff the actual quoted clause against the defect you logged before reading the operator's summary of what changed; fix one paragraph landing does not imply the others did.
