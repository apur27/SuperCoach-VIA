...
def generate_team_paragraphs(
    ...
    lines: List[str] = []
    # H4 here: the parent section "## AFL insights > ### YEAR season — live team analysis"
    # is at H3, so this and the per-team sub-headings below sit at H4.
...
def build_section_body(
    ...
    body.append(intro)
    body.append("")
    # H4 sub-headings — the parent "### YEAR season — live team analysis" sits at H3 inside
    # the AFL insights section, so all of its sub-sections are H4.
...
def render_5year_section(profile: pd.DataFrame, years: List[int], current_year: int) -> str:
    ...
    for _, row in profile.sort_values("team").iterrows():
        # H4 here: parent "### Team playing styles — 5 years of data" sits
        # at H3 inside AFL insights, so each team is H4.
...
def replace_5year_section(readme_text: str, current_year: int, years: List[int], body: str) -> str:
    ...
    Header level: this section sits at H3 under the H2 "## AFL insights"
    parent, so a freshly inserted section uses ### (not ##). The TOC entry
    is correspondingly indented as a nested item.
    """
...
def replace_section(readme_text: str, year: int, body: str) -> str:
    ...
    # No markers — find existing section by H3 header (the section now lives
    # at H3 under the "## AFL insights" parent).
    ...
    # Find the next H3-or-higher header after this section. We search for
    # `\n### ` (sibling) and `\n## ` (parent) — whichever comes first.
    after_idx = idx + len(header_token)
    next_h3 = readme_text.find("\n### ", after_idx)
    next_h2 = readme_text.find("\n## ", after_idx)
    candidates = [n for n in (next_h3, next_h2) if n != -1]
    next_idx = min(candidates) if candidates else len(readme_text)

    # Build new section: keep header at H3, wrap with markers, replace body.
    new_section = (
...
def main() -> None:
    ...
    print("[10/10] Regenerating data-visualisation charts (radar, heatmap, scatter)...")
