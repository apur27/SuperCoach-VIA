...
| Yearly top-100 CSV (season-end cadence) | `yearly_export_rows`, `history/yearly_top_100/<season>.json` | `test_analytics_rankings.py` | **gap**: browser JSON is published, but the yearly CSV download is not yet emitted by the release builder |
...
| `supercoach/era_based_statistical_analysis.py`, `scripts/era_boundary_threshold.py` | `analytics.eras` (`era_stats`, `yearly_trends`, `adjacent_era_tests`, `team_scoring`) | `test_analytics_eras.py`; parity delta ~3e-14 | **gap**: computed and tested, but not yet emitted as a release resource |
| Brownlow proxy (`docs/afl-brownlow-2026.md` generator) | `analytics.awards.brownlow_proxy` with a versioned ineligibility source | `test_analytics_awards.py` | **gap**: not yet emitted as a release resource |
