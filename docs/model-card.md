...
- Training time on the reference box (4 vCPU, `threads=4`): **about 106 s** for `scvia forecast` without `--replay-season`, uncontended, of which out-of-fold fitting is 42 s, feature building 11 s and the final fit 15 s. An independent second machine measured 108 s. The first run took 1,575 s (out-of-fold fitting 1,360 s) only because it ran at the same time as the integration suite, which trains its own 4-thread models. OpenMP oversubscription on 4 vCPUs slowed the out-of-fold fits 32-fold. Both runs produced a byte-identical bundle (payload sha256 `57c754f6…`). Don't run two training jobs at once on this box. A refresh reuses a cached bundle whose cache key (snapshot, cutoff, features, folds, versions, seed) is unchanged, and it never retunes implicitly.
