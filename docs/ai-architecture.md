...
| **Hand-off contracts are advisory, not enforced** **[gap]** | **Med** | The "Standard pre-match pipeline" diagram in §10 shows BriefBuilder → Scientist → FootyStrategy → DataSentinel → Skeptic. In practice each agent is invoked manually via `@"<Agent> (agent)"`. Nothing in the harness checks that DataSentinel actually ran on a draft before it was committed, or that Skeptic's report was `PASS` rather than `BLOCK`. | Stamp a hidden `<!-- council-pipeline: BriefBuilder@<sha>, Scientist@<sha>, FootyStrategy@<sha>, DataSentinel:PASS@<ts>, Skeptic:PASS@<ts> -->` block at the end of every council-authored doc. Pre-commit hook (see Gap 1) refuses commits where the stamp is missing or where any tier reports a non-PASS verdict. |
