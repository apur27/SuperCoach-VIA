...
<div align="center">
  ...
  <img src="https://img.shields.io/badge/data-2026%20season%20round%2022-green">
...
| Player performance files | **[data]** 13,360 | `data/player_data/` (one CSV per player, one row per game) |
| Backtest window | **[data]** R1–R22, 2026 | `data/prediction/backtest/` |
| Player-round predictions scored | **[data]** 7,898 | walk-forward backtest |
| Mean absolute error (disposals) | **[data]** 3.951 | player-weighted across all rounds |
...
**Plain English:** the model misses a player's next-round disposal count by about four disposals on average — usable signal on a 0–45 range, measured honestly across 7,898 predictions. The known weak spot is the elite tier, where error runs roughly 2.5× the global figure.
...
130 years of AFL history, structured. Every match since 1897, and **[data]** 13,360 individual player files — one CSV per player, a row for every game they ever played. A scraper refreshes it weekly so the numbers stay current.
...
Walk-forward backtest, 2026 season, Rounds 1–22. For each round the model is retrained using only data from before that round, predicts every player who played, and is scored against actuals.

| Window | Player-rounds | MAE | Within 5 | Within 10 | Bias |
|---|--:|--:|--:|--:|--:|
| **R1-R22 player-weighted** | **[data]** 7,898 | **[data]** 3.951 | **[data]** 74.4% | **[data]** 95.8% | **[data]** -0.120 |
...
**Technical:** the model is essentially unbiased in aggregate. The known failure mode is the elite tier — top-10-player MAE runs ~2.5x the global figure, driven by a residual ceiling effect and context (tag absorption, role rotations) the feature set captures only partially. Team-level signed bias spans **[data]** -0.64 (St Kilda, most under-predicted) to **[data]** +0.41 (West Coast, most over-predicted), with mean absolute team bias **[data]** 0.25 disposals.

Full per-round table (all 22 rounds), team-level breakdown for every club, biggest misses per round, and pre-registered methodology: **[docs/afl-backtest-2026.md](docs/afl-backtest-2026.md)**.
